...
check("Topic Create and Edit robots share one form-listbox tri-state owner", sharedSeoPanel.match(/<AdminFormListboxSelect/g)?.length === 2 && !sharedSeoPanel.includes("function RobotsSelect") && !sharedSeoPanel.includes("<select") && seoPanel.includes('robotsIndexListbox: "topic-robots-index-listbox"') && seoPanel.includes('robotsFollowListbox: "topic-robots-follow-listbox"') && seoPanel.includes('robotsIndexFocusTarget: "topic-robots-index"') && seoPanel.includes('robotsFollowFocusTarget: "topic-robots-follow"') && sharedSeoPanel.match(/sizing="full"/g)?.length === 2 && !sharedSeoPanel.includes("searchable="));
...
check("SEO issues have explicit correction targets", seoPanel.includes("TOPIC_SEO_CORRECTION_TARGETS") && seoPanel.includes('targetId: "topic-seo-title"') && seoPanel.includes('targetId: "topic-content-markdown"') && seoPanel.includes('targetId: "topic-image-alt"'));
...
check("review overview exposes publish, featured, dates, readiness, and warnings without owning submitted fields", review.includes("data-topic-publish-review-overview") && ["حالة النشر", "الحالة المميزة", "تاريخ النشر", "آخر تحديث", "نتيجة المراجعة", "التحذيرات"].every((label) => review.includes(`label="${label}"`)) && review.includes('checked(form, "is_published"') && review.includes('checked(form, "is_featured"') && review.includes("formatAuditTimestamp(updatedAt)") && editEditor.includes("updatedAt={topic.updated_at}") && editEditor.includes("featured={Boolean(topic.is_featured)}") && !review.includes('name="updated_at"') && !review.includes('name="created_at"'));
