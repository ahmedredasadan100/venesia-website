...
check("publish validation enforces the final 60 and 160 ceilings through the shared contract", !publishValidation.includes("> 70") && !publishValidation.includes("> 170") && publishValidation.includes("ENTITY_SEO_LIMITS.title.max") && publishValidation.includes("ENTITY_SEO_LIMITS.description.max") && saveAction.includes("validateEntitySeoValues(payload)") && helpers.includes("readEntitySeoFormData(formData)"));
...
check("publishing and display controls preserve field parity through shared owners", !/<AdminFormSwitch\b/.test(publishingOptions) && !/<AdminFormSwitch\b/.test(displaySettings) && ["is_featured", "is_popular"].every((name) => publishingOptions.includes(`name="${name}"`)) && publishingOptions.includes('name="status"') && ["show_title_on_page", "show_image_on_page", "show_excerpt_on_page"].every((name) => displaySettings.includes(`name="${name}"`)));
...
check("publishing actions render before review in create and edit", [createEditor, editEditor].every((source) => source.indexOf("<ContentPublishingOptions") < source.indexOf("<TopicPublishChecklistPanel")));
...
check("review overview exposes publish, featured, dates, readiness, and warnings without owning submitted fields", review.includes("data-topic-publish-review-overview") && ["حالة النشر", "الحالة المميزة", "تاريخ النشر", "آخر تحديث", "نتيجة المراجعة", "التحذيرات"].every((label) => review.includes(`label="${label}"`)) && review.includes('field(form, "status"') && review.includes('checked(form, "is_featured"') && review.includes("formatAuditTimestamp(updatedAt)") && editEditor.includes("updatedAt={topic.updated_at}") && editEditor.includes("featured={Boolean(topic.is_featured)}") && !review.includes('name="updated_at"') && !review.includes('name="created_at"'));
