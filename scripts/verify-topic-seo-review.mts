...
check("canonical and robots UI offer per-topic override plus global inheritance", ["canonical_url", "robots_index", "robots_follow"].every((name) => seoPanel.includes(`\"${name}\"`)) && sharedSeoPanel.includes("الإعداد العام"));
...
check("each Topic SEO submitted field has one adapter mapping and one shared render source", ["seo_title", "seo_description", "focus_keyword", "seo_keywords", "canonical_url", "robots_index", "robots_follow"].every((name) => seoPanel.match(new RegExp(`"${name}"`, "g"))?.length === 1) && ["seoTitle", "seoDescription", "focusKeyword", "canonicalUrl", "robotsIndex", "robotsFollow"].every((name) => sharedSeoPanel.match(new RegExp(`name=\\{fieldNames\\.${name}\\}`, "g"))?.length === 1));
