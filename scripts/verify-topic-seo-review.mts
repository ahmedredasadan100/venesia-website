...
const correctionButton = read("src/components/admin/content/editors/ContentCorrectionButton.tsx");
...
check("correction button is non-submitting and dispatches the shared content-editor event", correctionButton.includes('type="button"') && correctionButton.includes("navigateContentEditor") && !correctionButton.includes("window.location"));
...
check("review issues map to shared basic, FAQ, rich-media, and SEO targets", review.includes("ContentCorrectionButton") && ["content-title", "topic-faq-editor", "video_url", "gallery-editor", "content-seo-title"].every((target) => contentReviewCapability.includes(target)));
check("one Entity SEO contract owns the recommended title and description ranges", seoStandards.includes("ENTITY_SEO_LIMITS") && entitySeoContract.includes("min: 45, max: 60") && entitySeoContract.includes("min: 120, max: 160") && [sharedSeoPanel, seoScore].every((source) => source.includes("SEO_LENGTH_STANDARDS") && source.includes("assessSeoLength")) && [contentReviewCapability, saveAction].every((source) => source.includes("ENTITY_SEO_LIMITS")));
...
check("publishing decisions form two grouped cards inside the four-unit responsive dashboard", ["publication-schedule", "featured-popular"].every((id) => publishingOptions.includes(`data-content-review-decision="${id}"`)) && review.includes("md:grid-cols-2") && review.includes("lg:grid-cols-[minmax(0,1.55fr)_minmax(0,1.15fr)_minmax(0,0.85fr)_minmax(0,1fr)]") && publishingOptions.includes('name="status"') && publishingOptions.includes('name="is_featured"') && publishingOptions.includes('name="is_popular"') && publishingOptions.includes("TopicDateLabelField") && !["publication-status", "featured", "popular", "publish-date"].some((id) => publishingOptions.includes(`data-content-review-decision="${id}"`)));
...
check("Create and Edit review use one shared inline-expansion dashboard", review.includes("data-content-review-capability") && review.includes('data-content-review-presentation="dashboard"') && review.includes("data-content-review-analysis") && review.includes("data-content-review-issue") && review.includes("aria-expanded={expanded}") && !review.includes("AdminSingleOpenAccordion") && !review.includes('presentation?: "default" | "embedded"'));
check("review overview exposes grouped publishing, visibility, display, and score without owning submitted fields", ["publication-schedule", "featured-popular"].every((id) => publishingOptions.includes(`data-content-review-decision="${id}"`)) && review.includes('data-content-review-decision="score"') && review.includes('data-content-review-decision="display-settings"') && review.includes('checked(form, "is_featured"') && review.includes('checked(form, "is_popular"') && review.includes("formatAuditTimestamp(updatedAt)") && editEditor.includes("updatedAt={topic.updated_at}") && !review.includes('name="updated_at"') && !review.includes('name="created_at"'));
check("review keeps content, image, SEO, and Validation cards in semantic order", review.indexOf('id: "content"') < review.indexOf('id: "image"') && review.indexOf('id: "image"') < review.indexOf('id: "seo"') && review.indexOf('id: "seo"') < review.indexOf('id: "validation"'));
check("review separates advisory geometry from blocking Validation geometry", review.includes("GUIDANCE_ANALYSIS_CARDS = ANALYSIS_CARDS.slice(0, 3)") && review.includes("VALIDATION_ANALYSIS_CARD = ANALYSIS_CARDS[3]") && review.includes('data-content-review-analysis-tier={variant === "validation" ? "blocking" : "guidance"}') && review.includes("md:grid-cols-[minmax(0,1fr)_minmax(15rem,auto)_auto]") && review.includes("const publicationBlocked = errors > 0"));
check("review uses the exact Fix action and advisory-score wording", correctionButton.includes("إصلاح") && review.includes("الدرجة إرشادية ولا تمنع النشر وحدها."));
