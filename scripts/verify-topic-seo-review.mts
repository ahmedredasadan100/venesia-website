...
const seoBasicsStart = seoPanel.indexOf("const seoBasicsContent");
const seoReturnStart = seoPanel.indexOf("  return (", seoBasicsStart);
const seoPrimaryRender = seoPanel.slice(seoBasicsStart, seoReturnStart);
const seoHelperStart = seoPanel.indexOf("<AdminSingleOpenAccordion", seoReturnStart);
const seoHelperRender = seoPanel.slice(seoHelperStart);
...
check("Topic Create and Edit robots share one form-listbox tri-state owner", seoPanel.match(/<AdminFormListboxSelect/g)?.length === 2 && !seoPanel.includes("function RobotsSelect") && !seoPanel.includes("<select") && seoPanel.match(/name="robots_index"/g)?.length === 1 && seoPanel.match(/name="robots_follow"/g)?.length === 1 && seoPanel.includes('focusTargetId="topic-robots-index"') && seoPanel.includes('focusTargetId="topic-robots-follow"') && seoPanel.match(/sizing="full"/g)?.length === 2 && seoPanel.includes('props.robotsIndex === null ? "" : String(props.robotsIndex)') && seoPanel.includes('props.robotsFollow === null ? "" : String(props.robotsFollow)') && !seoPanel.includes("searchable="));
check("Topic Create and Edit enforce the shared Index, Follow, Canonical presentation order", editorRobotsIndexPosition >= 0 && editorRobotsIndexPosition < editorRobotsFollowPosition && editorRobotsFollowPosition < editorCanonicalPosition && seoPanel.includes('data-admin-seo-control-order="index-follow-canonical"') && seoPanel.includes("data-topic-seo-top-row") && seoPanel.includes('dir="rtl"') && seoPanel.includes("lg:grid-cols-[minmax(180px,.7fr)_minmax(180px,.7fr)_minmax(0,1.6fr)]") && !seoPanel.includes("flex-wrap"));
...
check("Create and Edit adopt one shared two-column SEO presentation", [createEditor, editEditor].every((source) => source.match(/<SeoPanel\b/g)?.length === 1 && !/(?:^|[<\s])presentation=/m.test(source)) && !/(?:^|\s)presentation\??:/.test(seoPanel) && !/(?:^|\s)presentation\s*===/.test(seoPanel) && seoPanel.match(/<AdminFormLayout/g)?.length === 1 && seoPanel.match(/<AdminSingleOpenAccordion/g)?.length === 1 && seoPanel.includes('defaultOpenId="search-result-preview"') && ["search-result-preview", "open-graph-preview", "live-seo-analysis"].every((id) => seoPanel.includes(`id: "${id}"`)) && ["معاينة نتائج البحث", "معاينة Open Graph", "تحليل SEO المباشر"].every((label) => seoPanel.includes(`label: "${label}"`)) && ["الشكل المتوقع للرابط داخل نتائج محركات البحث.", "الصورة والنص المتوقعان عند مشاركة الرابط.", "الدرجة الإرشادية وملاحظات تحسين المحتوى."].every((description) => seoPanel.includes(`description: "${description}"`)) && !["seo-basics", "social-sharing", "analysis-preview"].some((id) => seoPanel.includes(`id: "${id}"`)));
check("retired native-robots and all-fields-in-accordion Topic paths are absent", !seoPanel.includes("function RobotsSelect") && !seoPanel.includes("<RobotsSelect") && !seoPanel.includes("<select") && !seoPanel.includes('presentation === "editor"') && seoPanel.match(/<AdminSingleOpenAccordion/g)?.length === 1);
...
check("all Topic SEO editable fields remain always visible in the primary column", seoBasicsStart >= 0 && seoReturnStart > seoBasicsStart && seoHelperStart > seoReturnStart && ["seo_title", "seo_description", "focus_keyword", "seo_keywords", "robots_index", "robots_follow"].every((name) => seoPrimaryRender.includes(`name="${name}"`) && !seoHelperRender.includes(`name="${name}"`)) && seoPrimaryRender.includes("{canonicalField}") && !seoHelperRender.includes("{canonicalField}") && !seoPrimaryRender.includes("openGraphPreviewContent") && !seoPrimaryRender.includes("liveSeoAnalysisContent"));
check("shared accordion owns helper disclosure state, descriptions, target reveal, and accessibility", adminUiIndex.includes('from "./AdminSingleOpenAccordion"') && accordion.includes("description?: ReactNode") && accordion.includes("navigationEventName?: string") && accordion.includes("item.description") && accordion.includes("useState<string | null>") && accordion.includes("items[0]?.id ?? null") && accordion.includes("openId !== null") && accordion.includes("setOpenId((current) => (current === item.id ? null : item.id))") && accordion.includes('target?.closest<HTMLElement>("[data-admin-accordion-panel]")') && accordion.includes("setOpenId(itemId)") && accordion.includes('data-admin-single-open-accordion-active={resolvedOpenId ?? ""}') && accordion.includes('type="button"') && accordion.includes("aria-expanded={open}") && accordion.includes("aria-controls={panelId}") && accordion.includes('role="region"') && accordion.includes("aria-labelledby={triggerId}") && accordion.includes("hidden={!open}") && ["ArrowDown", "ArrowUp", "Home", "End"].every((key) => accordion.includes(`"${key}"`)) && accordion.includes("text-start") && !seoHelperRender.includes("navigationEventName="));
