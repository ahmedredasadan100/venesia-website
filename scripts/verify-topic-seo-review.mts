...
check("publishing actions keep balanced responsive controls", publishingOptions.includes("sm:grid-cols-2") && publishingOptions.includes("xl:grid-cols-4") && publishingOptions.includes('name="status"') && publishingOptions.includes('name="is_featured"') && publishingOptions.includes('name="is_popular"') && publishingOptions.includes("TopicDateLabelField"));
...
check("publishing actions render before review in create and edit", [createEditor, editEditor].every((source) => source.indexOf("<ContentPublishingOptions") < source.indexOf("<ContentReviewPanel")));
check("Create and Edit review use the shared SEO-style accordion", review.includes("data-content-review-capability") && review.includes("<AdminSingleOpenAccordion") && review.includes("data-content-review-issue") && !review.includes('presentation?: "default" | "embedded"'));
check("review overview exposes publish, featured, popular, dates, display, and score without owning submitted fields", ["الحالة", "المميز / الشائع", "تاريخ النشر", "إعدادات العرض", "آخر تحديث"].every((label) => review.includes(`label="${label}"`)) && review.includes('field(form, "status"') && review.includes('checked(form, "is_featured"') && review.includes('checked(form, "is_popular"') && review.includes("formatAuditTimestamp(updatedAt)") && editEditor.includes("updatedAt={topic.updated_at}") && !review.includes('name="updated_at"') && !review.includes('name="created_at"'));
check("review keeps validation, guidance, and passed checks in semantic order", review.indexOf('id: "review-errors"') < review.indexOf('id: "review-improvements"') && review.indexOf('id: "review-improvements"') < review.indexOf('id: "review-passed"'));
