...
check("topic switch consumers preserve one shared DOM and field contract", topicSwitch.match(/<AdminFormSwitch\b/g)?.length === 1 && !topicSwitch.includes("<input") && !topicSwitch.includes("uncheckedValue") && topicSwitch.includes("id={id}") && sharedSwitch.match(/type="checkbox"/g)?.length === 1 && sharedSwitch.includes("{uncheckedValue !== undefined ? (") && sharedSwitch.includes('<input type="hidden" name={name} value={uncheckedValue} />') && ["id={id}", 'type="checkbox"', 'role="switch"', "name={name}", "checked={checked}", "onChange={onChange}", "disabled={disabled}"].every((marker) => sharedSwitch.includes(marker)) && ["is_featured", "is_popular", "is_published"].every((name) => publishingOptions.includes(`name="${name}"`)) && /<TopicFormSwitch[\s\S]*?id="topic-published-switch"[\s\S]*?name="is_published"/.test(publishingOptions) && !publishingOptions.includes('<div id="topic-published-switch"') && ["show_title_on_page", "show_image_on_page", "show_excerpt_on_page"].every((name) => displaySettings.includes(`name="${name}"`)));
