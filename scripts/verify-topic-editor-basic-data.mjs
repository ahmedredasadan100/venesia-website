...
check("shared listbox exposes its non-record empty selection as an active descendant", listbox.includes("allowEmptySelection?: boolean") && listbox.includes('allowEmptySelection && placeholder') && listbox.includes('? [{ value: "", label: placeholder }, ...options]') && listbox.includes("const resolvedActiveValue = selectableOptions[activeIndex]?.value;") && listbox.includes("resolvedActiveValue === undefined") && (listbox.match(/resolvedActiveValue !== undefined/g)?.length ?? 0) >= 3);
...
check("all three Topic modes use one asymmetric compact Select geometry", basicPanel.includes('presentation="compact"') && categorySelect.includes('sizing="medium"') && seriesFields.includes('sizing="wide"') && typeControl.includes('sizing="content-relaxed"') && listbox.includes('sizing === "content-relaxed"') && listbox.includes('"min-w-32 w-fit max-w-full ps-4 pe-4"') && listbox.includes('sizing === "medium"') && listbox.includes('"min-w-60 w-full max-w-full ps-4 pe-4"') && listbox.includes('sizing === "wide"') && listbox.includes('"min-w-64 w-full max-w-full ps-4 pe-4"') && listbox.includes('const contentSized = sizing === "content" || sizing === "content-relaxed"') && listbox.includes('? "block w-full max-w-full"') && [typeControl, categorySelect, seriesFields].every((source) => source.includes("AdminListboxSelect")));
check("empty taxonomy controls use shared non-record placeholders while database records remain the only supplied options", categorySelect.includes('placeholder="اختر التصنيف"') && seriesFields.includes('placeholder="اختر السلسلة"') && seriesFields.includes("showPlaceholderForEmptyValue") && seriesFields.includes("allowEmptySelection") && seriesFields.includes("options.map((option)") && !seriesFields.includes('label: "بدون سلسلة"') && listbox.includes("showPlaceholderForEmptyValue") && listbox.includes("allowEmptySelection"));
...
check("Topic Create and Edit row three fills the desktop track with unequal aligned Content Type, Category, and Series controls", basicPanel.includes("data-topic-compact-select-row") && basicPanel.includes("grid gap-5 lg:grid-cols-[max-content_minmax(15rem,1fr)_minmax(16rem,1.08fr)] lg:items-end") && [typeControl, basicPanel, seriesFields].every((source) => source.includes('inline-grid min-w-0 max-w-full shrink-0 space-y-1.5')) && listbox.includes("flex h-11") && /\{contentTypeField\}\s*\{categoryField\}\s*\{seriesField\}/.test(basicPanel) && !basicPanel.includes("lg:grid-cols-3"));
...
check("Topic Create and Edit display settings use one equal compact three-cell owner", displaySettings.includes('<AdminFormSwitchGroup layout="equal-grid">') && displaySettings.match(/<TopicFormSwitch/g)?.length === 3 && displaySettings.match(/\ssurface\s/g)?.length === 3 && !displaySettings.includes("grid grid-cols-2") && !displaySettings.includes("presentation") && adminSwitch.includes('layout?: "flow" | "equal-grid"') && adminSwitch.includes("grid gap-4 sm:grid-flow-col sm:auto-cols-fr") && adminSwitch.includes('surface ? "grid grid-cols-[minmax(0,1fr)_auto] gap-3"') && /surface \? \(\s*<>\s*\{labelContent\}\s*\{switchControl\}/.test(adminSwitch) && !displaySettings.includes('name="show_faq_on_page"') && !basicPanel.includes("showFaq="));
