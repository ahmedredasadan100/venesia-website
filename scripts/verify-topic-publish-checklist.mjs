...
check("Review exposes the requested hero and advisory score wording", ["مراجعة قبل النشر", "تحقق من جاهزية المحتوى قبل نشره.", "درجة جاهزية النشر", "الدرجة إرشادية ولا تمنع النشر وحدها."].every((token) => review.includes(token)));
...
check("dashboard hero reuses the one form save and close owners", ["نشر الآن", "حفظ المسودة", "إجراءات أخرى"].every((label) => review.includes(label)) && review.includes('[data-admin-form-action="save"]') && review.includes('[data-admin-form-action="close"]') && review.includes("form.requestSubmit"));
