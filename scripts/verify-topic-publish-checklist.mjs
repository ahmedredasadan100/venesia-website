...
check("guidance analyses use one equal three-column row while Validation owns the full row below", review.includes("GUIDANCE_ANALYSIS_CARDS") && review.includes("VALIDATION_ANALYSIS_CARD") && review.includes("lg:grid-cols-3") && review.includes("data-content-review-guidance-grid") && review.includes("data-content-review-validation-row") && review.includes('variant="guidance"') && review.includes('variant="validation"') && !review.includes("lg:grid-cols-2\" data-content-review-analysis-grid"));
check("Validation presentation distinguishes blocking state without a new engine", ["تحقق مانع للنشر", "النشر ممنوع", "النشر مسموح", "مشكلة مانعة تحتاج إصلاحًا", 'data-content-review-analysis-tier={variant === "validation" ? "blocking" : "guidance"}'].every((token) => review.includes(token)) && review.includes('item.blocksPublish && item.status === "fail"') && review.includes("items={blockingIssues}") && review.includes("<ReviewIssueList items={issues} />"));
...
check("general notes and the read-only status summary are separate cards", review.includes("ملاحظات عامة") && review.includes("ملخص الحالة") && !review.includes("سجل المراجعة") && review.includes("data-content-review-notes") && review.includes("data-content-review-status-summary") && review.includes("TimelineEntry"));
...
check("publish blocking is explicit and independent from presentation severity", capability.includes("blocksPublish: boolean") && capability.includes("getContentPublishBlockingChecks") && capability.includes('item.blocksPublish && item.status === "fail"') && capability.includes("blocksPublish: false"));
