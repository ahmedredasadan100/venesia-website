...
check("Review starts directly with shared quick decisions and retains advisory score wording", ["مراجعة قبل النشر", "تحقق من جاهزية المحتوى قبل نشره.", "نشر الآن", "حفظ المسودة", "إجراءات أخرى"].every((token) => !review.includes(token)) && ["قرارات سريعة", "درجة جاهزية النشر", "الدرجة إرشادية ولا تمنع النشر وحدها."].every((token) => sharedReview.includes(token)));
...
check("the four analyses are independent cards with inline-only details", ["content", "image", "seo"].every((id) => review.includes(`id: "${id}"`)) && review.includes("جاهزية المحتوى") && review.includes("جاهزية الصورة وAlt") && review.includes("تحليل SEO") && sharedReview.includes('id: "validation"') && sharedReview.includes("التحقق العام (Validation)") && sharedReview.includes("data-admin-entity-review-expanded") && sharedReview.includes("aria-expanded={expanded}") && !sharedReview.includes("Drawer") && !sharedReview.includes("Dialog"));
