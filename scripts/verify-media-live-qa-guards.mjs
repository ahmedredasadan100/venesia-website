...
check("browser harness requires authoritative dataset readiness before its first mutation", sources.browser.includes("Pre-mutation Media Library") && sources.browser.includes("runtimeDatasetMatches") && sources.browser.includes("usageResultsAuthoritative") && sources.browser.includes("safeDeleteReady") && sources.browser.indexOf("readinessProbe = await context.request.get") < sources.browser.indexOf("topic = await createDraftTopic"));
...
check("browser harness waits for the upload control instead of racing initial library loading", sources.browser.includes("Media upload control readiness") && sources.browser.includes('name: "رفع ملفات"') && sources.browser.indexOf("Media upload control readiness") < sources.browser.indexOf("const uploadResponsePromise"));
check("browser harness proves a managed upload preserves authoritative readiness and zero live usage", sources.browser.includes("Post-upload Media Library") && sources.browser.includes("Post-upload Live Usage") && sources.browser.includes("Managed upload did not remain registered and ready"));
...
check("browser harness proves automatic post-save refresh with persisted and live zero usage", sources.browser.includes("automaticLibraryRefresh") && sources.browser.includes("automaticUsageRefresh") && sources.browser.includes("Persisted References did not become zero") && sources.browser.includes("Post-unlink automatic Live Usage refresh") && sources.browser.indexOf("const automaticLibraryRefresh") < sources.browser.indexOf("await saveTopicAndWaitForPost(unlinkPage"));
check("browser harness proves same-run Catalog, Storage, namespace and zero-reference identity before delete", sources.browser.includes("assertFixtureDeleteProof") && sources.browser.includes('select("id,provider,bucket,object_key,public_url,status")') && sources.browser.includes('select("domain_key,entity_type,entity_identity")') && sources.browser.includes("Catalog identity does not exactly match") && sources.browser.indexOf("await assertFixtureDeleteProof") < sources.browser.indexOf("clickSafeDeleteAndCapture(scenarioPage, authority, 200)"));
check("browser harness forbids Settings and reconciliation during the governing workflow", sources.browser.includes("forbiddenGoverningRequests") && sources.browser.includes('url.pathname === "/admin/settings/media"') && sources.browser.includes('operation === "reconcile"') && sources.browser.includes("Governing workflow used Settings or reconciliation"));
...
check("browser upload proof mirrors production filename normalization", sources.browser.includes("storageSafeNamespace") && sources.browser.includes('result.displayName !== originalFilename') && sources.browser.includes('\\d{13}-[0-9a-f-]{12}'));
check("browser cleanup discovery uses the same normalized Storage identity", sources.browser.includes('const stem = storageSafeNamespace(originalFilename.replace') && sources.browser.includes('like("object_key", `images/${stem}-%`)') && sources.browser.includes('list("images", { search: stem'));
check("governing Browser mode skips reservation and Recovery UI branches", sources.browser.includes('scenario !== "standard" && scenario !== "governing"') && sources.browser.includes('const governingOnly = scenario === "governing"') && sources.browser.includes('if (!governingOnly) {\n      await runRecoveryUiChecks'));
