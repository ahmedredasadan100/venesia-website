...
const heroConsumer = read(
  "src/components/projects/ProjectsHubCanonicalHero.tsx",
);
const projectRenderer = read(
  "src/components/projects/details/ResidentialProjectDetails.tsx",
);
const commercialProjectRenderer = read(
  "src/components/projects/details/CommercialProjectDetails.tsx",
);
...
const projectDistrict = read(
  "src/components/projects/details/ProjectDistrictSection.tsx",
);
const plansRenderer = read(
  "src/components/projects/details/ProjectPlansAndAreasSection.tsx",
);
const deliveryRenderer = read(
  "src/components/projects/details/ProjectDeliverySpecsSection.tsx",
);
const galleries = read(
  "src/components/projects/details/ProjectImageGalleries.tsx",
);
...
const mobileProjectOverlays = read(
...
const titleMigration = read(
  "sql/migrations/20260817100000_project_section_title_contract.sql",
);
const mediaMigration = read(
  "sql/migrations/20260817101000_media_ordinary_attachment_scope.sql",
);
const safeDeleteMigration = read(
  "sql/migrations/20260725180000_media_delete_reservation_saga.sql",
);
const projectDetailHeroMigration = read(
  "sql/migrations/20260823100000_project_detail_hero_configuration_adoption.sql",
);

assert.equal(
  existsSync(resolve(root, "src/components/projects/ProjectsHubHero.tsx")),
  false,
...
assert.doesNotMatch(
  heroConfig,
  /arabicName|englishName|heroImage|shortDescription/,
...
assert.match(
  heroAdapter,
  /sortProjectsByHomepageOrder\(projects\)/,
...
for (const token of [
  ...
  assert.ok(
    projectDetailsHero.includes(token),
    `Project Detail Hero adoption missing ${token}`,
  );
}
...
assert.match(projectDetailsHero, /projectDetailsMainClassName/);
assert.match(projectDetailsHero, /showProjectHero \? "" : "pt-\[106px\]"/);
assert.match(projectRenderer, /projectDetailsMainClassName\(showProjectHero\)/);
assert.match(
  commercialProjectRenderer,
  /projectDetailsMainClassName\(showProjectHero\)/,
);
assert.match(
  heroAdminEditor,
  /const contentTab = isProjectDetail \?[\s\S]*showName="show_eyebrow"/,
  "the existing Hero Admin screen must expose Project Detail presentation controls",
);
assert.match(
  heroAdminActions,
  /isDomainBackedHeroTemplateVariant\(variant\)[\s\S]*imageComposition: base\.imageComposition/,
  "domain-backed Hero persistence must exclude copied Project content",
);
assert.match(
  heroAdminActions,
  /const status = parseFormStatus\(formData\);[\s\S]*is_visible: status === "published"/,
  "Hero publication must persist the visibility field consumed by the existing replacement RPC",
);
assert.match(
  projectDetailHeroMigration,
  /hero_templates_project_detail_singleton_idx/,
...
assert.doesNotMatch(
  projectRenderer,
  /showDistrictLocationLabel|showLocationLabel=/,
...
assert.match(
  homeProjectsConfig,
  /readShowFlag\(\s*"showProjectLocation",\s*"show_project_location",?\s*\)/,
...
assert.match(
  contentActions,
  /showProjectLocation:\s*parseFormBoolean\(\s*formData,\s*"show_project_location",\s*false,?\s*\)/,
  "Home Project Cards must persist the presentation decision through the existing module config owner",
);
assert.match(
  homeProjectsRenderer,
  /showProjectLocation\s*\?\s*project\.location\.label\s*:\s*null/,
...
for (const hardCodedCopy of [
  ...
  assert.equal(
    [projectRenderer, plansRenderer, deliveryRenderer].some((source) =>
...
assert.match(
  galleries,
  /slice\(safeGroupIndex \* 4, safeGroupIndex \* 4 \+ 4\)/,
);
assert.match(
  galleries,
  /slice\(safeGroupIndex \* 3, safeGroupIndex \* 3 \+ 3\)/,
);

assert.match(
  publicLoader,
  /\.eq\("slug", slug\)\s*\.eq\("publication_status", "published"\)/,
);
assert.match(
  publicLoader,
  /PUBLIC_PROJECT_MODEL_CACHE_VERSION/,
...
assert.match(
  publicMapper,
  /plainText === RETIRED_DELIVERY_PRESENTATION_COPY \? "" : body/,
);
assert.doesNotMatch(
  projectsTable,
...
const leaseBody = mediaMigration.slice(
  mediaMigration.indexOf(
