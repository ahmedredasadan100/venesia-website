-- Complete the existing Admin Pages read model with database-owned search.
-- This changes only the RPC contract; no Page or assignment data is mutated.
begin;

do $$
begin
  if to_regprocedure('public.admin_list_pages(integer,integer,text,text)') is null
     and to_regprocedure('public.admin_list_pages(integer,integer,text,text,text)') is null then
    raise exception 'admin_list_pages owner is missing';
  end if;
end;
$$;

drop function if exists public.admin_list_pages(integer, integer, text, text);
...
comment on function public.admin_list_pages(integer, integer, text, text, text) is
  'Service-role Admin Pages read model owning literal search, assignment counts, sorting, normalized pagination, and total count.';
