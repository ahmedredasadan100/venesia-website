-- Atomic sync for project child rows (admin save).
-- NULL payload = skip section; JSON array (including []) = replace section.
...
begin
  ...
  if p_floor_plans is not null then
    ...
    for v_item in select value from jsonb_array_elements(p_floor_plans) loop
      if coalesce(nullif(btrim(v_item->>'area'), ''), '') = ''
         and coalesce(nullif(btrim(v_item->>'label'), ''), '') = ''
         and coalesce(nullif(btrim(v_item->>'plan_image'), ''), '') = '' then
        continue;
      end if;
  ...
  if p_delivery_items is not null then
    ...
    for v_item in select value from jsonb_array_elements(p_delivery_items) loop
      if coalesce(nullif(btrim(v_item->>'body'), ''), '') is null then
        continue;
      end if;
  ...
  if p_overview_media is not null then
    ...
    for v_item in select value from jsonb_array_elements(p_overview_media) loop
      if coalesce(nullif(btrim(v_item->>'image'), ''), '') is null then
        continue;
      end if;
