...
function IntegrationOverviewVisual() {
  ...
  return (
    <div className="relative mx-auto h-[188px] w-full max-w-[270px] overflow-hidden rounded-[28px] border border-white/9 bg-[radial-gradient(circle_at_center,rgba(216,184,122,.12),transparent_58%)]" aria-hidden="true">
      ...
      <svg viewBox="0 0 270 188" className="absolute inset-0 h-full w-full text-[#D8B87A]/30" fill="none">
        <path d="M57 47 135 94l78-47M57 141l78-47 78 47" stroke="currentColor" strokeWidth="1.2" strokeDasharray="4 7" />
        <circle cx="135" cy="94" r="46" stroke="currentColor" strokeWidth="1" strokeDasharray="2 8" />
      </svg>
      <div className="absolute left-1/2 top-1/2 grid size-20 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-[26px] border border-[#D8B87A]/28 bg-[#D8B87A]/[0.10] text-[#E4C683] shadow-[0_0_70px_rgba(216,184,122,.14)]">
        <svg viewBox="0 0 48 48" className="size-10" fill="none">
          <circle cx="13" cy="14" r="5" stroke="currentColor" strokeWidth="2.2" />
          <circle cx="35" cy="14" r="5" stroke="currentColor" strokeWidth="2.2" />
          <circle cx="24" cy="35" r="5" stroke="currentColor" strokeWidth="2.2" />
          <path d="m17 17 4 13m10-13-4 13M18 14h12" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" />
        </svg>
      </div>
      {satellites.map((item) => (
        <div key={item.key} className={`absolute scale-[.68] ${item.className}`}>
...
function MetricCard({
  ...
  return (
    <article className={`min-h-[118px] rounded-[22px] border bg-gradient-to-bl to-transparent p-4 shadow-[0_18px_55px_rgba(0,0,0,.18)] ${toneClasses}`}>
...
function DisabledAction({ children, reason }: { children: string; reason: string }) {
  return (
    <button
      type="button"
      disabled
...
function IntegrationAction({ href, children }: { href: string | null; children: string }) {
  if (!href) {
    return <DisabledAction reason="يتطلب Connection Adapter آمنًا ومفعّلًا داخل المالك الحالي.">{children}</DisabledAction>;
  }
  return (
    <Link
      href={href}
    ...
    <article className="group relative flex min-h-[300px] flex-col overflow-hidden rounded-[24px] border border-white/9 bg-gradient-to-bl from-white/[0.045] to-[#080B10]/78 p-5 shadow-[0_22px_70px_rgba(0,0,0,.20)] transition hover:-translate-y-1 hover:border-[#D8B87A]/25">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1 pt-1">
          <p className="font-en truncate text-[15px] font-semibold text-white">{item.label}</p>
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <AdminStatusPill tone={STATUS_TONES[item.status]}>
              <span className={`ms-1.5 inline-block size-1.5 rounded-full ${STATUS_DOTS[item.status]}`} aria-hidden="true" />
              {STATUS_LABELS[item.status]}
            </AdminStatusPill>
            <span className="text-[9px] font-semibold text-[#D8B87A]/55">{CATEGORY_LABELS[item.category]}</span>
          </div>
        </div>
      </div>

      <p className="mt-4 min-h-11 text-[11px] leading-6 text-white/42">{item.description}</p>
...
export default function AdminIntegrationsPlatform({ snapshot }: { snapshot: IntegrationsSnapshot }) {
  ...
  const visibleIntegrations = useMemo(
    () => snapshot.integrations.filter((item) => {
      const matchesQuery = !normalizedQuery || [item.label, item.description, CATEGORY_LABELS[item.category]]
        .join(" ")
    ...
    <AdminPageExperience className="min-w-0 pb-12" dir="rtl">
      ...
      >
        <div className="grid items-center gap-5 xl:grid-cols-[280px_minmax(0,1fr)]">
          <div>
            <h2 id="integrations-overview-heading" className="text-lg font-semibold text-white">نظرة عامة</h2>
            <p className="mt-2 text-xs leading-6 text-white/42">حالة الاتصال والمزامنة كما تعيدها الملاك المعتمدة، دون أرقام تجريبية أو نجاح افتراضي.</p>
            <div className="mt-4"><IntegrationOverviewVisual /></div>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
            <MetricCard label="إجمالي التكاملات" value={snapshot.statistics.total} helper="منصات المرحلة الأولى فقط" tone="gold" />
            <MetricCard label="المتصل" value={snapshot.statistics.connected} helper="اتصال مثبت بمصدر حقيقي" tone="green" />
      ...
      <section aria-labelledby="available-integrations-heading" className="space-y-4">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div>
              <div className="flex items-center gap-3">
                <h2 id="available-integrations-heading" className="text-lg font-semibold text-white">التكاملات المتاحة</h2>
                <span className="rounded-full border border-[#D8B87A]/18 bg-[#D8B87A]/[0.06] px-2.5 py-1 font-en text-[10px] text-[#E4C683]">{visibleIntegrations.length}/{snapshot.integrations.length}</span>
              </div>
              <p className="mt-1 text-[11px] leading-5 text-white/35">البحث والتصفية يعملان على كتالوج المرحلة الأولى دون تغيير مصدر الحقيقة.</p>
            </div>
            ...
            <div className="grid flex-1 gap-3 sm:grid-cols-2 xl:max-w-[820px] xl:grid-cols-[minmax(250px,1fr)_170px_170px_auto]">
              <label className="relative block sm:col-span-2 xl:col-span-1">
                ...
                <span className="pointer-events-none absolute inset-y-0 right-4 grid place-items-center text-white/32"><SearchIcon /></span>
                <input
                  type="search"
                  value={query}
              ...
              <label>
                <span className="sr-only">فلترة حسب الفئة</span>
                <select
                  value={category}
                  onChange={(event) => setCategory(event.target.value as CategoryFilter)}
                  className="min-h-11 w-full cursor-pointer rounded-xl border border-white/10 bg-[#0B0E13] px-4 text-xs text-white/65 outline-none focus:border-[#D8B87A]/42 focus:ring-2 focus:ring-[#D8B87A]/12"
                >
              ...
              <label>
                <span className="sr-only">فلترة حسب حالة الاتصال</span>
                <select
                  value={status}
        ...
        <p className="sr-only" aria-live="polite">تم العثور على {visibleIntegrations.length} تكامل</p>
        {visibleIntegrations.length ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {visibleIntegrations.map((item) => <IntegrationCard key={item.key} item={item} />)}
          </div>
        ) : (
          <div className="rounded-[26px] border border-dashed border-white/12 bg-white/[0.02] px-6 py-14 text-center">
            <p className="text-sm font-semibold text-white/65">لا توجد تكاملات تطابق البحث الحالي</p>
            <p className="mt-2 text-xs leading-6 text-white/38">غيّر الفلاتر أو أعد تعيينها لعرض المنصات التسع المعتمدة.</p>
            <button type="button" onClick={resetFilters} className="mt-5 cursor-pointer rounded-xl border border-[#D8B87A]/25 px-4 py-2 text-xs font-semibold text-[#E5CA91] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#D8B87A]">عرض كل التكاملات</button>
          </div>
        )}
      </section>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-sm font-semibold text-white">حالة أمان الاتصالات</p>
              <AdminStatusPill tone={securityReady ? "green" : "red"}>{securityReady ? "محمي" : "يحتاج مراجعة"}</AdminStatusPill>
            </div>
