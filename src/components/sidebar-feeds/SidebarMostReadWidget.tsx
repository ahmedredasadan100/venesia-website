...
import {
  ...
  type FeedPopularPresentationVariant,
...
type SidebarMostReadWidgetProps = {
  ...
  presentationVariant: FeedPopularPresentationVariant;
...
export default function SidebarMostReadWidget({
  ...
  const cards = items.map((item, index) => {
    ...
    const hasText = resolvedCardFormatting.showTitle || hasDate || hasExcerpt;

    return (
      <Link
        key={`${item.href}-${item.title}`}
        href={item.href}
        className="group flex h-full items-center gap-3 rounded-2xl border border-transparent p-2 transition hover:border-[#D8B87A]/25 hover:bg-[#D8B87A]/[0.04]"
  ...
  return (
    <SidebarFeedPanel eyebrow={eyebrow ?? undefined} title={title} formatting={formatting}>
      {presentationVariant.layout === "list" ? (
        ...
        <div
          className={`grid gap-4 ${feedGridColumnsClass(presentationVariant.columns)}`}
