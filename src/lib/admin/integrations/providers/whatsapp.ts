...
import { assertGrantedScopes, requireAsset, uniqueAssets } from "./shared";
import {
...
async function discoverWhatsApp(input: ProviderRuntimeContext) {
  const businesses = await collectMetaGraphPages<{ id?: string; name?: string }>(`${metaGraphBase()}/me/businesses?fields=id,name&limit=100`, input.accessToken, "whatsapp_business_discovery_failed");
  ...
  for (const business of businesses) {
    ...
    const wabas = await collectMetaGraphPages<Record<string, unknown>>(`${metaGraphBase()}/${business.id}/owned_whatsapp_business_accounts?fields=id,name,currency,timezone_id,message_template_namespace&limit=100`, input.accessToken, "whatsapp_waba_discovery_failed");
    for (const waba of wabas) {
      ...
      const phones = await collectMetaGraphPages<Record<string, unknown>>(`${metaGraphBase()}/${wabaId}/phone_numbers?fields=id,display_phone_number,verified_name,quality_rating,code_verification_status&limit=100`, input.accessToken, "whatsapp_phone_discovery_failed");
...
export const whatsappBusinessAdapter: IntegrationProviderAdapter = {
  ...
  configuration() {
    return resolveIntegrationApplicationConfiguration("whatsapp_business");
  },
  testApplicationConfiguration() {
    return testMetaApplicationConfiguration("whatsapp_business");
  },
  async buildAuthorizationRequest(context) {
    const credentials = await requireMetaApplicationCredentials("whatsapp_business");
  ...
  async exchangeAuthorizationCode(input): Promise<ProviderTokenSet> {
    ...
    const me = await providerJson<{ id?: string }>(`${metaGraphBase()}/me?fields=id`, { headers: headers(accessToken) }, "whatsapp_subject_lookup_failed");
    return {
  ...
  async testConnection(input) {
    try {
      ...
      await providerJson(`${metaGraphBase()}/${phone.externalId}?fields=id,display_phone_number,verified_name,quality_rating,code_verification_status`, { headers: headers(input.accessToken) }, "whatsapp_connection_test_failed");
  ...
  async syncAnalytics(input) {
    const phone = requireAsset(input.assets, "phone_number");
    const payload = await providerJson<Record<string, unknown>>(`${metaGraphBase()}/${phone.externalId}?fields=id,display_phone_number,verified_name,quality_rating,code_verification_status`, { headers: headers(input.accessToken) }, "whatsapp_health_sync_failed");
    return {
