...
export async function generateMetadata({
  ...
  const item = await getMediaItemBySlug("press", slug);
  ...
  return buildMetadata({
    path: `/media-center/press/${item.slug}`,
...
export default async function DetailsPage({
  ...
  const pagePath = `/media-center/press/${item.slug}`;
  ...
  const pageJsonLd = buildPageJsonLd({
    ...
    breadcrumbs: [
      ...
      { name: "البيانات الصحفية", path: "/media-center/press" },
        ...
        <article className="space-y-10">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-[1.5rem] border border-[#D8B87A]/20 bg-[#D8B87A]/[0.07] p-5">
            <p className="text-sm leading-7 text-white/65">
              تابع البيانات والتغطيات الرسمية داخل المركز الإعلامي.
            </p>
          ...
          <RelatedMediaRail
            ...
            items={relatedItems}
            getHref={(relatedItem) => `/media-center/press/${relatedItem.slug}`}
