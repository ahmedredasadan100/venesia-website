...
function mergeSubmittedContentFormatting<T extends Record<string, unknown>>(
  formData: FormData,
  config: T,
): T & PageBlockTextFormattingConfig {
  const submittedFields = CONTENT_FORMATTING_FIELDS.filter(
    (field) =>
      formData.has(`${field}_alignment`) ||
      formData.has(`${field}_bold`) ||
      formData.has(`show_${field}`),
  );
  return {
    ...config,
    ...buildPageBlockTextFormattingPatch(
      formData,
      submittedFields.map((field) => ({ field })),
    ),
  };
}
...
function readContacts(
  formData: FormData,
): NonNullable<AboutCtaModuleConfig["contacts"]> {
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    const secondaryValue = cleanText(
      formData.get(`contact_${index}_secondary_value`),
    );
    const icon = cleanText(formData.get(`contact_${index}_icon`));
    const linkData = linkFieldFromFormData(formData, `contact_${index}`);
    if (!label && !value && !secondaryValue && !hasSavedLinkField(linkData))
      continue;
    contacts.push({
...
function buildAboutCtaConfig(formData: FormData): AboutCtaModuleConfig {
  ...
  return {
    ...
    button: {
      label: cleanText(formData.get("button_label")) || undefined,
      ...(buttonLink
        ? { link: buttonLink.link, target: buttonLink.target }
        : {}),
    },
...
function readPrincipleItems(formData: FormData) {
  const count = Math.min(
    6,
    Math.max(0, parseNumber(formData.get("principle_count")) ?? 0),
  );
  ...
  return Array.from({ length }, (_, index) => ({
    ...
    imageAlt:
      cleanText(formData.get(`principle_${index}_image_alt`)) || undefined,
  }));
}

function buildAboutPrinciplesConfig(
  formData: FormData,
  ...
  if (cleanText(formData.get("include_home_trust_intro")) === "1") {
    config.description =
      cleanText(formData.get("principles_intro")) || undefined;
    ...
    config.eyebrowAlignment =
      eyebrowAlignRaw === "right" ||
      eyebrowAlignRaw === "left" ||
      eyebrowAlignRaw === "center"
        ? eyebrowAlignRaw
        : "right";
...
function buildAboutApproachConfig(
  formData: FormData,
...
function buildHomeProjectsConfig(formData: FormData): HomeProjectsModuleConfig {
  ...
  const projectsLimit =
    limitText && Number.isFinite(parsedLimit) && parsedLimit > 0
  ...
  return {
    ...
    showProjectLocation: parseFormBoolean(
      formData,
    ...
    projectsLimit,
    cardCtaLabel: cleanText(formData.get("card_cta_label")) || "استكشف المشروع",
    cardCtaAlignment,
    eyebrowBold,
    eyebrowAlignment,
    footerCta,
...
function buildProjectsHubHeroTypedConfig(
  formData: FormData,
): ProjectsHubHeroModuleConfig {
  const selectionMode = cleanText(
    formData.get("selection_mode"),
  ) as ProjectsHubHeroSelectionMode;
  ...
  if (
    !(PROJECTS_HUB_HERO_PROJECT_TYPES as readonly string[]).includes(
      projectType,
    )
  ) {
  ...
  const autoplayMs = assertAutoplayMs(
    Number(cleanText(formData.get("autoplay_ms")) || 6000),
  );
  const emptyRaw = cleanText(formData.get("empty_state"));
  const emptyState = emptyRaw
    ? assertSafePlainText(emptyRaw, "نص الحالة الفارغة", 400)
    : null;
...
function buildProjectsHubFeaturedTypedConfig(
  formData: FormData,
): ProjectsHubFeaturedModuleConfig {
  const selectionMode = cleanText(
    formData.get("selection_mode"),
  ) as ProjectsHubFeaturedSelectionMode;
  if (!PROJECTS_HUB_FEATURED_SELECTION_MODES.includes(selectionMode)) {
    throw new Error("قاعدة اختيار المشروعات المميزة غير مدعومة.");
  }
  const title = assertSafePlainText(
    cleanText(formData.get("title")) || "مشروع مميز",
    "العنوان",
    120,
  );
  ...
  return {
    ...
    showProjectDescription: parseFormBoolean(
      formData,
...
function buildProjectsHubListingTypedConfig(
  formData: FormData,
): ProjectsHubListingModuleConfig {
  ...
  const defaultFilter = cleanText(
    formData.get("default_filter"),
  ) as ProjectsHubFilterId;
  if (!visibleFilters.includes(defaultFilter)) {
    throw new Error("الفلتر الافتراضي يجب أن يكون ضمن الفلاتر الظاهرة.");
  }
  const defaultView = cleanText(
    formData.get("default_view"),
  ) as ProjectsHubViewMode;
  ...
  return {
    eyebrow: assertSafePlainText(
      cleanText(formData.get("eyebrow")) || "Projects Index",
      "النص التمهيدي",
      80,
    ),
    title: assertSafePlainText(
      cleanText(formData.get("title")) || "جميع المشروعات",
      "عنوان القسم",
      120,
    ),
    showEyebrow: parseFormBoolean(formData, "show_eyebrow", false),
...
async function buildProjectsHubMapTypedConfig(
  formData: FormData,
): Promise<ProjectsHubMapModuleConfig> {
  const pinCount = Math.min(
    30,
    Math.max(0, Number(cleanText(formData.get("pin_count")) || 0)),
  );
  ...
  const knownCodes = new Set(
    (projectRows ?? []).map((row) => String(row.code).toUpperCase()),
  );
  ...
  return {
    title: assertSafePlainText(
      cleanText(formData.get("title")) || "مشروعاتنا على الخريطة",
      "العنوان",
      120,
    ),
    mapImage: assertSafeCmsMediaPath(
      cleanText(formData.get("map_image")),
      "صورة الخريطة",
    ),
...
function resolveStructuredVariant(slug: string, variantInput: string | null) {
  if (isAboutIntroSingleImageTemplate(slug, variantInput))
  ...
  if (usesAboutPrinciplesConfigSchema(slug, variantInput))
    return "about-principles";
  ...
  if (isProjectsHubFeaturedTemplate(slug, variantInput))
    return "projects-hub-featured";
...
async function buildContentConfig(
  ...
  if (isProjectsHubTemplate(resolvedSlug, variantInput)) {
    ...
    if (
      ...
      return mergeProjectsHubConfig(
        existingConfig,
        mergeSubmittedContentFormatting(
          formData,
  ...
  if (
    schema === "about-intro" ||
    usesAboutIntroConfigSchema(resolvedSlug, variantInput)
  ) {
    return mergeSubmittedContentFormatting(
      formData,
      buildAboutIntroConfig(formData),
    );
  }
  if (
    schema === "about-intro-single-image" ||
    ...
    return mergeSubmittedContentFormatting(
      formData,
      buildAboutIntroSingleImageConfig(formData),
    );
  }
  if (
    schema === "vision-goals" ||
    isVisionGoalsTemplate(resolvedSlug, variantInput)
  ) {
    return mergeSubmittedContentFormatting(
      formData,
      buildVisionGoalsConfig(formData),
    );
  }
  if (
    schema === "about-cta" ||
    usesAboutCtaConfigSchema(resolvedSlug, variantInput)
  ) {
    return mergeSubmittedContentFormatting(
      formData,
      buildAboutCtaConfig(formData),
    );
  }
  if (
    schema === "about-principles" ||
    usesAboutPrinciplesConfigSchema(resolvedSlug, variantInput)
  ) {
    return mergeSubmittedContentFormatting(
      formData,
      buildAboutPrinciplesConfig(formData),
    );
  }
  if (
    schema === "about-approach" ||
    isAboutApproachTemplate(resolvedSlug, variantInput)
  ) {
    return mergeSubmittedContentFormatting(
      formData,
      buildAboutApproachConfig(formData),
    );
  }
  if (
    schema === "home-projects" ||
    isHomeProjectsTemplate(resolvedSlug, variantInput)
  ) {
    return mergeSubmittedContentFormatting(
      formData,
      buildHomeProjectsConfig(formData),
    );
  }
  return mergeSubmittedContentFormatting(
    formData,
...
async function ensureUniqueSlug(slug: string, id?: number) {
  ...
  if (error)
    throw new Error(`Content module slug lookup failed: ${error.message}`);
  return !data;
}
...
export async function createContentBlock(
  ...
  if (!name)
    return createContentBlockFailure(revision, "اسم البلوك مطلوب.", "name");
  if (!slug)
    return createContentBlockFailure(
      revision,
      "اكتب slug صالحًا للبلوك.",
      "slug",
    );
  ...
  try {
    ...
    const coordinated = await coordinateMediaReferenceEntityMutation({
      ...
      mutate: async () => {
        const { data, error } = await getSupabaseAdmin()
          ...
          .single();
        if (error || !data)
          throw new Error(error?.message ?? "تعذر إنشاء بلوك المحتوى.");
        return data;
      },
    ...
    const message =
      error instanceof Error
        ? error.message
        : "تعذر إنشاء بلوك المحتوى. حاول مرة أخرى.";
    if (createdId) {
      return createContentBlockSuccess(
        revision,
        createdId,
...
export async function updateContentBlock(formData: FormData) {
  ...
  if (existingError || !existing)
    throw new Error(existingError?.message || "البلوك غير موجود.");
  ...
  const slugLocked = isStructuralContentTemplateSlug(
    existing.slug,
    existing.variant,
  );
  ...
  if (
    isProjectsHubTemplate(existing.slug, existing.variant) &&
    !isProjectsHubTemplate(slug, cleanText(formData.get("variant")) || null)
  ) {
...
export async function deleteContentBlock(formData: FormData) {
  ...
  if (mediaSynchronization.status === "saved_with_media_sync_warning") {
    try {
      ...
      console.error(
        "Content block delete committed with a Media synchronization warning; cache revalidation also failed.",
        revalidationError,
      );
    }
    redirect(
      "/admin/pages-blocks/blocks/content?notice=saved_with_media_sync_warning",
    );
  }
  await revalidateBlockModulePaths("content");
}
...
export async function duplicateContentBlock(formData: FormData) {
  ...
  const coordinated = await coordinateMediaReferenceEntityMutation({
    ...
    mutate: async () => {
      ...
      if (insertError || !data)
        throw new Error(
          insertError?.message ?? "Unable to duplicate content block.",
        );
      return data;
    },
    resolveEntityIdentity: (value) => String(value.id),
...
export async function bulkContentBlocks(formData: FormData) {
  ...
  if (mediaSynchronization?.status === "saved_with_media_sync_warning") {
    try {
      ...
      console.error(
        "Content block bulk delete committed with a Media synchronization warning; cache revalidation also failed.",
        revalidationError,
      );
    }
    redirect(
...
export async function getContentBlockRows(): Promise<ContentBlockRow[]> {
  ...
  if (error) throw new Error(error.message);
  return (data ?? []).filter(
    (row) => !isRetiredContentBlockTemplateSlug(row.slug),
  );
}
