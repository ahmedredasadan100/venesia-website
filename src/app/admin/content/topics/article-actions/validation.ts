...
export async function getTopicById(id: string) {
  ...
  const { data, error } = await getSupabaseAdmin()
    .from("topics")
    .select(
      "id, title, slug, excerpt, content, image, image_alt, media_payload, category_id, category_slug, series_id, status, published_at, published_by, date_label, updated_at, deleted_at, seo_title, seo_description, focus_keyword, seo_keywords, canonical_url, robots_index, robots_follow, og_image, og_image_alt, faq, seo_score, seo_score_version, seo_score_input_hash, show_title_on_page, show_image_on_page, show_excerpt_on_page, show_date_on_page, show_category_on_page, show_series_on_page, show_intro_card_on_page, show_faq_on_page, show_faq_title_on_page",
    )
    .eq("id", Number(id))
