...
export async function updateTopicWithStatus(
  ...
  if (validationMode === "publish") {
    ...
    const category = await getCategory(payload.categorySlug, currentTopic.category_slug);
    if (!category) {
      const categoryError = await getCategoryValidationError(payload.categorySlug, currentTopic.category_slug);
    ...
    const { error } = await getSupabaseAdmin()
      .from("topics")
      .update({
        ...buildTopicWritePayload(writePayload, category, series, statusToWrite, now, currentTopic),
        updated_by: actor.id,
        ...(statusToWrite === "published" && currentStatus !== "published"
    ...
    redirect(redirectToList ? `/admin/content/topics?notice=${notice}` : `/admin/content/topics/${id}?notice=${notice}`);
  }
  ...
  const { error } = await getSupabaseAdmin()
    .from("topics")
    .update({
      ...buildTopicWritePayload(payload, category, series, nextStatus, now, currentTopic),
      updated_by: actor.id,
