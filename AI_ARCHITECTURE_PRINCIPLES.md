...
Position is a Page Assignment decision. Page Composition declares a semantic Region contract independently of Slug, page name, Template, Theme, CSS, or current Layout; every page exposes that contract, and flexible modules inherit its Positions. A Theme MUST map those Regions onto its own visual layout without rewriting or narrowing the CMS contract. A module MAY declare a Product-fixed Position only when its semantic nature requires it, such as Hero; an implementation preference is not sufficient reason to constrain Position.

Page Composition MAY select, order, and place a registered renderer. It MUST remain unaware of that renderer's image geometry, internal flow, typography, and presentation variants. Presentation remains derived from the module's current Product contract; Page Composition does not persist or infer it. Shared Module Presentation is a presentation contract under the existing Design System/shared-component boundary; it is not a Runtime, Capability, or second composition owner.
