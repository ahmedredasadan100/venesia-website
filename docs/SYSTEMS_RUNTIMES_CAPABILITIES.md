...
| Search Platform Module          | CMS-managed search launchers and `/search` results presentation                                | Search Content Module presentation schema                 | Search Runtime, engine, read owner, or placement owner        |
...
| Page Composition and Menus      | Specialized domain owners with aggregate atomic mutation RPCs. **Page Composition:** stable semantic Regions and their meaning; module assignment eligibility for those Regions; each assignment's Position; each assignment's order; each assignment's visibility. **Theme:** the outer Layout; the outer Container; the outer Grid; available and responsive dimensions; visual Region mapping; spacing between page Regions and modules. **Module:** internal visual composition; image placement and aspect ratio; internal content distribution and semantic order; visual identity; Product variants. | Theme renderers consume the complete semantic assignment contract; Theme never redefines Regions, and Page Composition never owns outer geometry or internal Module presentation. |
| Projects Domain                 | Database-only project truth with atomic aggregate persistence, publication, row actions, and public read contracts; Project location values are data only                                                                                                                                                                          | Each Module owns its presentation decision. The Project Location Section alone reads its two per-Project Boolean decisions through a sibling presentation payload; Hero, Featured, Listing, Home, Search, Tracking, JSON-LD, and other Consumers ignore them |
...
| Search Platform Module          | One Content Module schema owns search copy, scope, supported content types, result limit, available filters, default sort, and presentation; every launcher projects into the ordinary CMS Page `/search`                                                                                                                         | It reads only through Public Content Read, paginates only through Public Pagination, and cannot own placement, Navigation/Footer membership, another engine, or route-local search state                                     |
...
| Executable Governance           | Existing adoption manifests plus the Next compiled route manifest own typed applicability, executable binding proof, consumer reachability, and bidirectional route coverage across Collection/Form capabilities, Media writers, Global SEO, Content Editors, Page Composition preferences, Admin Users, and Platform Performance | Raw source tokens, regex discovery, filenames, unused imports, and absence-based defaults are not governance evidence                                        |
